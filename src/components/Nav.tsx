...
export default function Nav() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 bg-navy/95 backdrop-blur border-b border-white/10 transition-[padding] duration-300 ${
        scrolled ? "py-0" : "py-1"
      }`}
    >
      <div
        className={`max-w-6xl mx-auto px-6 flex items-center justify-between transition-[padding] duration-300 ${
          scrolled ? "py-2.5" : "py-4"
        }`}
      >
        <a href="/" className="flex items-center gap-3">
          {/* WA monogram: brushed-metal W with a nested gold A, replaces the old double-V polyline mark */}
          <Image src="/wex-icon-wa.png" alt="Wex Advisory" width={83} height={56} priority className="h-14 w-auto" />
